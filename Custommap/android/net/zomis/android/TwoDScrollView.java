...
public class TwoDScrollView extends FrameLayout {
	...
	private boolean	mIgnoreMultiTouch = true;
	...
	private boolean canScroll() {
		if (!this.isScrollEnabled()) return false;
...
//		CustomFacade.getLog().v("scroll - onTouch " + ev);
		...
		if (ev.getActionMasked() == MotionEvent.ACTION_DOWN && ev.getEdgeFlags() != 0) {
		...
		switch (action) {
			case MotionEvent.ACTION_POINTER_DOWN:
				if (mIgnoreMultiTouch) return false;
				break;
			case MotionEvent.ACTION_DOWN:
				/*
